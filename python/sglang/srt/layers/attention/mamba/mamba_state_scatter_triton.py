...
def split_qkv_rmsnorm_rope_kernel(
    ...
    # qgate
    weight_values = tl.load(q_weight_ptr + tl.arange(0, HEAD_DIM)) + 1
    ...
    for row_idx in tl.range(row_pid, batch_size, row_step):
        ...
        cat_q = tl.zeros((Q_BLOCK_SIZE // HEAD_DIM, HEAD_DIM), dtype=tl.float32)
        cat_q = tl.insert_slice(
            cat_q,
            roped_q,
            offsets=(0, 0),
            sizes=(Q_BLOCK_SIZE // HEAD_DIM, ROPE_DIM),
        ...
        tl.store(
            ...
            cat_q.reshape(Q_BLOCK_SIZE).to(q_ptr.dtype.element_ty),
            mask=valid_mask_out,
        )
        tl.store(
            gate_q_ptr + output_offset + col_indices_out,
            gate.reshape(Q_BLOCK_SIZE).to(gate_q_ptr.dtype.element_ty),
            mask=valid_mask_out,
        )
        input_offset += input_offset_step
        output_offset += output_offset_step
    # k
    weight_values = tl.load(k_weight_ptr + tl.arange(0, HEAD_DIM)) + 1
...
def split_qkv_gate_rmsnorm_rope(
    input,
    sin,
    cos,
    q_hidden_size,
    ...
    return q_output, k_output, v_output, q_gate_output