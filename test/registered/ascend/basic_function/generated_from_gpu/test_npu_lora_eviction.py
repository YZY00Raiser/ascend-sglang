...
# ==============================================================================
"""
NPU LoRA Eviction Test

This test verifies LoRA memory eviction behavior on NPU.
Test cases are ported from GPU test_lora_eviction.py for NPU compatibility.

[Test Category] Integration
[Test Target] LoRA memory eviction with different target modules on NPU
"""
...
class TestNpuLoRAEviction(CustomTestCase):
    """Test LoRA eviction behavior on NPU."""

    def test_npu_lora_eviction_with_different_target_modules(self):
        """
        Test LoRA eviction with different target modules on NPU.

        This test runs inference against two LoRA adapters in different orders
        to force eviction behavior, and ensures that the outputs of the same
        (adapter, prompt) pair are consistent across runs.
        """
        output_history = {}
    ...
    def test_npu_lora_eviction_with_reused_lora_name(self):
        """
        Test LoRA eviction with reused LoRA names on NPU.

        This test runs inference against two LoRA adapters with the same name
        to ensure that the eviction behavior works correctly when reusing LoRA names.
        """
        output_history = {}
        adapters = [LORA_A, LORA_B]
    ...
    def _run_test(
        ...
        """Run eviction test with specified configuration."""
        ...
        # Initialize runner with NPU-specific settings
        with SRTRunner(
            base_path,
            ...
            for i in range(repeat):
                for j, lora_path in enumerate(adapter_sequence):
                    print(
                        f"\n========== Testing LoRA eviction with adapter '{lora_path}' "
                        f"(#{j + 1}/{len(adapter_sequence)}), reuse_lora_name: {reuse_lora_name}, "
                        f"reversed: {reverse}, repeat: {i + 1}/{repeat} ---"
                    )
                        ...
                        for prompt in PROMPTS:
                            ...
                            if prev_output is not None:
                                self.assertEqual(
                                    prev_output,
                                    output,
                                    f"Output mismatch for adapter {lora_path} and prompt '{prompt}' "
                                    f"on repeat {j + 1}, previous: '{prev_output}', current: '{output}'.",
                                )
                            else:
